...
$runnerPath = Join-Path $scriptDir "run_entry_next_open_cycle.py"

function Get-PythonCommand {
    if ($env:ENTRY_NEXT_OPEN_PYTHON -and (Test-Path $env:ENTRY_NEXT_OPEN_PYTHON)) {
        return [pscustomobject]@{
            Exe = $env:ENTRY_NEXT_OPEN_PYTHON
            PrefixArgs = @()
        }
    }
    ...
    throw "No usable Python executable found. Set ENTRY_NEXT_OPEN_PYTHON or install Python 3."
}
