"""
Measure whether official benchmark and sub-index context helps explain replay outcomes.

Usage:
    python replay_official_context_diagnostics.py REPLAY_ID
"""
import json
import math
import os
...
def _build_metric_summary(records, key_name):
    values = [
        {"id": record["record_id"], "value": record.get(key_name)}
    ...
    for bucket_index in sorted(grouped):
        ...
        summaries.append({
            ...
            "value_min": min(item[key_name] for item in bucket_records if item.get(key_name) is not None),
            "value_max": max(item[key_name] for item in bucket_records if item.get(key_name) is not None),
...
def _render_markdown(summary):
    lines = [
        "# Replay Official Context Diagnostics",
        "",
        f"- replay_id: `{summary['replay_id']}`",
        f"- built_at: `{summary['built_at']}`",
        f"- comparison_count: `{summary['comparison_count']}`",
        f"- records_with_official_benchmark: `{summary['records_with_official_benchmark']}`",
        f"- records_with_official_sector: `{summary['records_with_official_sector']}`",
        "",
        "## Notes",
        "",
    ]
    ...
    for metric_summary in summary.get("metric_summaries", []):
        ...
        for bucket in metric_summary.get("buckets", []):
            lines.append(
                f"- `{bucket['bucket']}`: count `{bucket['record_count']}`, actionable `{bucket['actionable_count']}`, "
...
def build_official_context_diagnostics(replay_id):
    ...
    for path in sorted(comparison_paths):
        ...
        official = (frozen_case.get("historical_context") or {}).get("official_context") or {}
        benchmark = official.get("benchmark_nepse_index") or {}
        sector = official.get("sector_index") or {}
        market_summary = official.get("market_summary") or {}
        records.append({
            ...
            "benchmark_turnover_value": benchmark.get("turnoverValue"),
            "sector_pct_change": sector.get("percentageChange"),
            "sector_turnover_value": sector.get("turnoverValue"),
            "market_total_transactions": market_summary.get("totalTransactions"),
        })

    notes = [
        "this diagnostic is descriptive only and does not change any replay rule",
        "official NEPSE history is rolling, so this diagnostic is only meaningful on March+ windows where the official context exists",
    ]
    metric_names = [
        "benchmark_pct_change",
        "benchmark_turnover_value",
        "sector_pct_change",
        "sector_turnover_value",
        "market_total_transactions",
    ]
    metric_summaries = [_build_metric_summary(records, metric) for metric in metric_names]
    ...
    summary = {
        ...
        "records_with_official_benchmark": sum(1 for item in records if item.get("benchmark_pct_change") is not None),
        "records_with_official_sector": sum(1 for item in records if item.get("sector_pct_change") is not None),
        "notes": notes,
    ...
    dated_json_path = os.path.join(
        summaries_dir,
        f"{datetime.now().strftime('%Y-%m-%d')}__official_context_diagnostics_v1.json",
    )
    latest_json_path = os.path.join(summaries_dir, "latest__official_context_diagnostics_v1.json")
    dated_md_path = os.path.join(
        summaries_dir,
        f"{datetime.now().strftime('%Y-%m-%d')}__official_context_diagnostics_v1.md",
    )
    latest_md_path = os.path.join(summaries_dir, "latest__official_context_diagnostics_v1.md")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python replay_official_context_diagnostics.py REPLAY_ID")
        sys.exit(1)

    replay_id = sys.argv[1]
    summary, latest_json_path, latest_md_path = build_official_context_diagnostics(replay_id)
    print(json.dumps({
        "replay_id": replay_id,
        "comparison_count": summary["comparison_count"],
        "records_with_official_benchmark": summary["records_with_official_benchmark"],
        "records_with_official_sector": summary["records_with_official_sector"],
